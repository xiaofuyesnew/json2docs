...
import { Document, DocumentElement, TextElement, HeadingElement, ParagraphElement, ListElement, TableElement, ImageElement, DividerElement, ContainerElement, DocumentStyle } from '../types';
...
export class HtmlGenerator extends BaseGenerator {
  async generate(document: Document): Promise<GenerationResult> {
    try {
      ...
      return this.createSuccessResult(outputPath, html);
    } catch (error) {
...
<body>
  ...
  private generateStyles(style?: DocumentStyle): string {
    const defaultStyles = `
        body {
  ...
  private renderTableElement(element: TableElement): string {
    const style = this.buildStyleString(element.style);
    const borderStyle = element.borderWidth ? `border: ${element.borderWidth} solid ${element.borderColor || '#ddd'};` : '';

    const headers = element.headers.map(header => `<th style="${borderStyle}">${header}</th>`).join('');
  ...
  private renderImageElement(element: ImageElement): string {
    const style = this.buildStyleString(element.style);
  ...
  private renderDividerElement(element: DividerElement): string {
    const style = this.buildStyleString(element.style);
  ...
  private renderContainerElement(element: ContainerElement): string {
    const style = this.buildStyleString(element.style);
